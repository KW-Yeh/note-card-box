...
import { ArrowLeft, Pencil, Trash2, Share2, Link as LinkIcon } from 'lucide-react';
  ...
  const loadCard = async () => {
    ...
    try {
      ...
      if (cardData) {
        ...
        setLinkedCards(linkedCardsData.filter((c): c is Card => c !== undefined));
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              ...
              <span>
                更新於 {formatDistanceToNow(card.updatedAt, { addSuffix: true, locale: zhTW })}
              </span>
            ...
            {totalLinks === 0 ? (
              ...
              <div className="space-y-2">
                {cardLinks.from.map((link) => {
                  const linkedCard = linkedCards.find((c) => c.id === link.targetId);
                ...
                {cardLinks.to.map((link) => {
                  const linkedCard = linkedCards.find((c) => c.id === link.sourceId);
                  if (!linkedCard) return null;
